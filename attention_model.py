...
def train(full_model, en_seq, sp_seq, batch_size, n_epochs=1):
    ...
    for ep in range(n_epochs):
        ...
        for bi in range(0, en_seq.shape[0] - batch_size, batch_size):
            ...
            l, acc = full_model.evaluate([en_onehot_seq, sp_onehot_seq[:, :-1, :]], sp_onehot_seq[:, 1:, :], batch_size=batch_size, verbose = 1)

            print('Epoch = ', ep , 'Iteration = ' , int(bi/batch_size)+1 , 'Accuracy = ', acc)
            losses.append(l)

            if bi%100 == 0:
                infer_dec_model.save_weights('decoder_weights.h5')
                infer_enc_model.save_weights('encoder_weights.h5')
...
en_vsize = 30000#max(en_tokenizer.index_word.keys()) + 1
